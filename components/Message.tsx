                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Object.entries(
                                        message.sourceChunks.reduce((acc, chunk) => {
                                            const key = chunk.documentName ? `${chunk.documentName}::${chunk.pageNumber}` : `Page ${chunk.pageNumber}`;
                                            if (!acc[key]) acc[key] = [];
                                            acc[key].push(chunk);
                                            return acc;
                                        }, {} as Record<string, typeof message.sourceChunks>)
                                    ).map(([key, chunksOnPage]) => {
                                        const [docName, pageNumStr] = key.split('::');
                                        const pageNum = pageNumStr ? parseInt(pageNumStr) : parseInt(docName.replace('Page ', ''));
                                        const displayName = docName.includes('Page ') ? docName : `${docName} (P${pageNum})`;
